...
def train_one_epoch(
    ...
    for batch in pbar:
        ...
        if own_max > 1000.0 or neighbor_max > 1000.0:
            print(f"\nEXTREME INPUT FEATURES at batch {len(losses)+1}!")
        ...
        if has_nan_grad or total_grad_norm > GRADIENT_THRESHOLD:
            print(f"\nSkipping batch {len(losses)+1}: grad_norm={total_grad_norm:.2f}")
            optimizer.zero_grad()
            skipped_batches += 1
            continue
    ...
    if skipped_batches > 0:
        print(f"    Skipped {skipped_batches} batches due to NaN/extreme gradients")
    ...
    if avg_metrics['loss_mse_component'] > 0 and avg_metrics['loss_corr_component'] > 0:
        ...
        if ratio < 0.1 or ratio > 10.0:
            print(f"    Loss imbalance detected: MSE/Corr ratio = {ratio:.2f}")
