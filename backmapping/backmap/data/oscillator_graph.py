...
def _extract_neighbor_beads(
    all_oscillators: Optional[List[Dict[str, Any]]],
    current_idx: int,
) -> Tuple[Optional[np.ndarray], Optional[np.ndarray]]:
    """Extract BB beads from neighboring oscillators for extended context."""
    if all_oscillators is None or len(all_oscillators) <= 1:
        return None, None
    
    bb_from_prev = None
    bb_from_next = None
    
    current_osc = all_oscillators[current_idx]
    current_type = current_osc.get("oscillator_type")
    
    if current_type == "backbone":
        curr_bb_curr_key = current_osc.get("bb_curr_key")
        curr_bb_next_key = current_osc.get("bb_next_key")
    else:
        curr_bb_prev_key = current_osc.get("bb_prev_key")
        curr_bb_curr_key = None
        curr_bb_next_key = None
    
    if current_idx > 0:
        prev_osc = all_oscillators[current_idx - 1]
        if prev_osc.get("oscillator_type") == "backbone":
            prev_bb_next_key = prev_osc.get("bb_next_key")
            prev_bb_curr_key = prev_osc.get("bb_curr_key")
            
            if current_type == "backbone":
                if prev_bb_next_key not in [curr_bb_curr_key, curr_bb_next_key]:
                    bb_from_prev = prev_osc.get("bb_next")
                elif prev_bb_curr_key not in [curr_bb_curr_key, curr_bb_next_key]:
                    bb_from_prev = prev_osc.get("bb_curr")
            else:
                if prev_bb_next_key != curr_bb_prev_key:
                    bb_from_prev = prev_osc.get("bb_next")
                elif prev_bb_curr_key != curr_bb_prev_key:
                    bb_from_prev = prev_osc.get("bb_curr")
    
    if current_idx < len(all_oscillators) - 1:
        next_osc = all_oscillators[current_idx + 1]
        if next_osc.get("oscillator_type") == "backbone":
            next_bb_curr_key = next_osc.get("bb_curr_key")
            next_bb_next_key = next_osc.get("bb_next_key")
            
            if current_type == "backbone":
                if next_bb_curr_key not in [curr_bb_curr_key, curr_bb_next_key]:
                    bb_from_next = next_osc.get("bb_curr")
                elif next_bb_next_key not in [curr_bb_curr_key, curr_bb_next_key]:
                    bb_from_next = next_osc.get("bb_next")
            else:
                if next_bb_curr_key != curr_bb_prev_key:
                    bb_from_next = next_osc.get("bb_curr")
                elif next_bb_next_key != curr_bb_prev_key:
                    bb_from_next = next_osc.get("bb_next")
    
    return bb_from_prev, bb_from_next
...
def build_graph_from_oscillator(
    ...
    *,
    ...
    # Extract neighbor BB beads for extended context
    ...
    if osc_type == "backbone":
        ...
        # Add neighbor beads for extended context
        if bb_neighbor_prev is not None:
            if not _is_zero_pos(np.asarray(bb_neighbor_prev)):
                _add_bead("BB_neighbor", bb_neighbor_prev, 0)
        if bb_neighbor_next is not None:
        ...
        if bb_neighbor_next is not None:
            if not _is_zero_pos(np.asarray(bb_neighbor_next)):
                _add_bead("BB_neighbor", bb_neighbor_next, 0)
