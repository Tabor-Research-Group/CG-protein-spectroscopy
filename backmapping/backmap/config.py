    ...
    def from_dict(d: Dict[str, Any]) -> "Config":
        ...
        if isinstance(d, dict):
            ...
            if isinstance(train_d, dict):
                # Older configs used eval_every_epochs; the current field is val_every_epochs
                if "eval_every_epochs" in train_d and "val_every_epochs" not in train_d:
                    train_d["val_every_epochs"] = train_d["eval_every_epochs"]

                # Some configs used init under infer; keep it as-is (still supported)
                d["train"] = train_d
        ...
        def _sub(cls, key: str):
            ...
            if not isinstance(sub, dict):
                sub = {}
            # Filter unknown keys
            unknown = [k for k in sub.keys() if k not in cls.__dataclass_fields__]
            if unknown:
                print(f"[config] Warning: ignoring unknown keys in '{key}': {sorted(unknown)}")
